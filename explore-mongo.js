...
const main = async () => {
	...
	MongoClient.connect(url, {useNewUrlParser: true})
		.then(client => {
			...
			mongo.db.collection('block').countDocuments()
				.then(count => {
					console.log({count});
					if (count === 0) {
						// genesis block.hash
						nextblockhash = '000000000019d6689c085ae165831e934ff763ae46a2a6c172b3f1b60a8ce26f';
						handleBlock(nextblockhash);
					} else {
						mongo.db.collection('block').find().skip(count - 1).toArray()
							.then(block => {
								assert(block.length === 1);
								nextblockhash = block[0].nextblockhash;
								handleBlock(nextblockhash);
							});
					}
				});
			// const block = db.collection('block').find().skip(db.collection.count() - 1);
			// console.log (block);
			// client.close();
		})
		.catch(error => {
